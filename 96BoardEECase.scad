...
optional_connectors_length = 100;
optional_connectors_width = 2;
optional_connectors_thickness = 20;
...
total_thickness = board_thickness + board_top_clearence + board_top_clearance_extra_tolerance + board_bottom_clearence;
...
bd_top_clearence = board_top_clearence+board_top_clearance_extra_tolerance+EE_spec_tolerance;
...
optional_connectors_offset = ((X_invert_value-54)/2)+1;
DC_pwr_offset = X_invert_value - 21.00;
...
module 96BoardMountHoles(penetration){
    z_loc = (penetration == true)?-1*(case_wall_thickness+(EE_spec_tolerance/2)+(board_top_clearance_extra_tolerance/2)+cutout+.01):0;
...
module mount_cylinder(x, y, z, size, penetration){
    ...
    union() {
        ...
        cylinder(penetration != true?(bd_total_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance) * z_scaler:(bd_total_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance+(case_wall_thickness*2)+cutout) * z_scaler, d = size, false, $fn=smoothness);
        ...
        if (screw_taper == true && penetration == true) {
            color("Indigo")
            translate([x * x_scaler,y * y_scaler,(((bd_total_thickness+case_wall_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance)-screw_taper_height))* z_scaler])
            cylinder(screw_taper_height * z_scaler, r1=(size/2), r2=(size/2)+1.5, $fn=smoothness);  // screw taper
            color("red")
            translate([x * x_scaler,y * y_scaler,(bd_total_thickness+case_wall_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance) * z_scaler])
...
module 96BoardOuterCase(){
    if (rounded_case != true){
        draw_cube(0,0,0,
        board_length+EE_spec_tolerance+(case_wall_thickness*2),bd_width+EE_spec_tolerance+(case_wall_thickness*2),bd_total_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance+(case_wall_thickness*2),false);
    } else {
        draw_roundedBox((board_length+EE_spec_tolerance+(case_wall_thickness*2))/2,((bd_width+EE_spec_tolerance+(case_wall_thickness*2))/2),((bd_total_thickness+(case_wall_thickness*2))/2),
        board_length+EE_spec_tolerance+(case_wall_thickness*2), bd_width+EE_spec_tolerance+(case_wall_thickness*2), bd_total_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance+(case_wall_thickness*2), 5, only_rectangle_rounded);
    }
} 
...
module 96BoardBlock(penetration){
    ...
    difference() {
        draw_cube(0,0,0, board_length+EE_spec_tolerance, bd_width+EE_spec_tolerance, bd_total_thickness+EE_spec_tolerance+board_top_clearance_extra_tolerance, false);
...
module 96BoardBare(penetration){
    ...
    union() {
        ...
        translate([0,0,0]){
            ...
            // Optional Connectors, 
            color("DarkGreen")
            face_penetration("BA", optional_connectors_offset, optional_connectors_length, optional_connectors_width, optional_connectors_thickness, board_back_edge, board_top_surface, false);
            ...
            if (96Board_UART_Board_Installed == true){
                ...
                face_penetration("RS", uart_board_connector_offset, uart_board_connector_length, uart_board_connector_width,
